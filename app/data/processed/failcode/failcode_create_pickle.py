...
def main():
    ...
    # Read the Dataset ,'Legacy 500','Legacy 600','Lineage'
    ...
    for read in ['Phenom']:  
        ...
        # Limit the PROBLEM DESCRIPTION length to 300 characters
        df_training['PROBLEM DESCRIPTION'] = df_training['PROBLEM DESCRIPTION'].str.slice(0,300).head(len(df_training))
        
        # Eliminates leading and trailing spaces from each value of the FAILCODE DESCR column
        df_training['FAIL CODE DESCRIPTION'] = df_training['FAIL CODE DESCRIPTION'].str.strip()
        ...
        df_samples = df_training['FAIL CODE DESCRIPTION'].value_counts().to_frame()
        
        df_samples = df_samples.reset_index()
        df_samples.columns = ['FAIL CODE DESCRIPTION', 'SAMPLES']
        ...
        # Apply preprocessing in traning model data / holdout validation
        df_training_cutted_processed = do_preprocessing(df_training_cutted['PROBLEM DESCRIPTION'])

        # Apply preprocessing in cross validation
        x_val_processed = do_preprocessing(x_val['PROBLEM DESCRIPTION'])
        ...
        pl = { 'Phenom' :[
            ...
            ('vect', CountVectorizer(ngram_range=(1,2))),
            ('tfidf',TfidfTransformer()),
            ('clf',OneVsRestClassifier(LinearSVC( class_weight='balanced', max_iter=20000), n_jobs=-1))] ,
            'Legacy 600' :[
            ('vect', CountVectorizer(ngram_range=(1,2))),
            ('tfidf',TfidfTransformer()),
            ('clf',OneVsRestClassifier(LinearSVC(), n_jobs=-1))] ,
        ...
        score = cross_val_score(text_clf,x_val_processed, x_val['FAIL CODE DESCRIPTION'] , cv = k_fold)
        
        print(f'{program} x-val accuracy {np.mean(score)} (+/-) {score.std()*2}')

        # Fit the model
        text_clf.fit(df_training_cutted_processed, df_training_cutted['FAIL CODE DESCRIPTION'])

        # Predict the test data
        predicted_data = text_clf.predict(df_test['PROBLEM DESCRIPTION'])

        # Calculate the accuracy with holdout validation method
        accuracy = np.mean (predicted_data == df_test['FAIL CODE DESCRIPTION'])*100

        print(f'{program} accuracy: {accuracy}')

        dict_accuracies = {}
        for predicted, correct in zip(predicted_data, df_test['FAIL CODE DESCRIPTION']):
        ...
        df_accuracy.columns = ['FAIL CODE DESCRIPTION', 'ACCURACY']
